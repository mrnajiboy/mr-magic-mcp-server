...
- Even if you’re only using the stdio MCP server locally, you still need the HTTP automation server running to serve those `/downloads/:id/:ext` routes whenever `redis` storage is enabled.
- For local testing, set `MR_MAGIC_DOWNLOAD_BASE_URL=http://127.0.0.1:3333` (or `http://localhost:3333`) so the generated links look like `http://127.0.0.1:3333/downloads/<id>/<ext>`. If you override the server port via `PORT` or `--port`, update the base URL accordingly.
